...
| **登录管理** | Cookie 登录态检测、未登录提醒、自动等待登录 |
| **弹窗处理** | 已知弹窗自动关闭、任务间自动清理 |
| **坐标录制** | 运行时实时录制点击坐标、导出 JS 代码 |
| **场景检测** | 主界面/冒险/战斗等场景识别框架 |
| **智能战斗** | 像素采样检测画面静止，自动判断战斗结束 |

### 🚧 计划中

- 截图 OCR 辅助（画面状态检测，替代固定延时）
- 多分辨率适配测试
- 限时活动任务（丁次烤肉、樱花季等）
- 基于像素特征的场景自动识别
...
| 版本 | 日期 | 变更 |
|------|------|------|
| v0.2.1 | 2026-09-08 | 智能战斗检测（像素采样）、任务流程文档、开发指南 |
| v0.2.0 | 2026-09-08 | 登录检测、弹窗处理、坐标录制、场景检测框架、快捷操作 |
